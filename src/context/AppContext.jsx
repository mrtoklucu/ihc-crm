...
export const AppProvider = ({ children }) => {
  ...
  const addLead = (lead) => {
    const newLead = { ...lead, id: Date.now(), assigneeId: null, createdAt: new Date().toISOString() };
    setLeads([...leads, newLead]);
  };

  const assignLead = (leadId, userId) => {
    setLeads(leads.map(lead => lead.id === leadId ? { ...lead, assigneeId: userId } : lead));
  };
  ...
  return (
    <AppContext.Provider value={{
      ...
      login, logout, addLead, assignLead, addRole, addUser
    }}>
      {children}
    </AppContext.Provider>
  );
